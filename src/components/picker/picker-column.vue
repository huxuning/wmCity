<template>
  <div class="picker-column" :class="classNames">
    <div class="picker-column-wrapper" :class="{ dragging: isDragging }" ref="wrapper" :style="{ height: visibleContentHeight + 'px' }">
      <div v-for="(item, index) in currentValues" :key="index" class="picker-column-item" :class="{ 'picker-column-item-selected': item === currentValue }"
...
<script>
  import translateUtil from 'common/js/transition'
      ...
      initEvents () {
        ...
        draggable(el, {
          start: (event) => {
            ...
            pickerItems = el.querySelectorAll('.picker-item'); // eslint-disable-line
          },
          drag: (event) => {
